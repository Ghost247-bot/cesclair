...
export const CHANNEL = "_HOVER_v1" as const;
const VISUAL_EDIT_MODE_KEY = "_visual_edit_mode" as const;
const FOCUSED_ELEMENT_KEY = "_focused_element" as const;
...
// Helper to parse data--id to extract file path, line, and column
const parseId = (
  Id: string
...
export default function HoverReceiver() {
  ...
  const protectChildElements = (element: HTMLElement) => {
    ...
    childElements.forEach((child) => {
      ...
      childEl.setAttribute("data--protected", "true");
  ...
  const restoreChildElements = (element: HTMLElement) => {
    ...
    protectedElements.forEach((child) => {
      ...
      childEl.removeAttribute("data--protected");
  ...
  const handleTextChange = (element: HTMLElement) => {
    ...
    // Get the  ID from the element to ensure we're working with the right one
  ...
  useEffect(() => {
    function handleMessage(e: MessageEvent) {
      ...
      } else if (e.data?.type === "_IMAGE_UPDATE") {
  ...
  useEffect(() => {
    ...
    const handleMouseUp = () => {
      if (focusedElementRef.current && focusedElementId) {
        ...
        // Extract file info from data--id
        const Id = element.getAttribute("data--id");
        if (Id) {
          const parsed = parseId(Id);
          if (parsed) {
  ...
  useEffect(() => {
    ...
    function onPointerMove(e: PointerEvent) {
      ...
      const hit =
        document
          ...
          ?.closest<HTMLElement>("[data--id]") ?? null;
      ...
      if (hit !== lastHitElementRef.current) {
        ...
        const tagName =
          hit.getAttribute("data--name") || hit.tagName.toLowerCase();
    ...
    function onClickCapture(e: MouseEvent) {
      ...
      if (hit) {
        ...
        // Find ALL other elements with the same  ID and show hover boxes
        const allMatchingElements = document.querySelectorAll(
          `[data--id="${hitId}"]`
        ) as NodeListOf<HTMLElement>;
        ...
        // Clicked on empty space or element without data--id
