...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!session?.user || (session.user as any)?.role !== 'admin') {
      return NextResponse.json({ error: 'Only admins can upload hairstylist avatars', code: 'FORBIDDEN' }, { status: 403 });
    }
    ...
    if (file.size > maxSize) {
      return NextResponse.json({ error: 'File size exceeds 5MB limit', code: 'FILE_TOO_LARGE' }, { status: 400 });
    }

    const timestamp = Date.now();
    const randomString = Math.random().toString(36).substring(2, 15);
    const fileExtension = file.name.split('.').pop()?.toLowerCase() || 'jpg';
    const fileName = `hairstylist-avatar-${timestamp}-${randomString}.${fileExtension}`;
    ...
    console.error('Hairstylist avatar upload error:', error);
